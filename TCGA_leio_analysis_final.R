...
genes <- Signature_33
genes_2 <- HDACs_MEFs
heatmap_zscore_name <- 'Leio_Hmap_signature_33.png'
survival_plot_name <- 'Leio_Survival_curve_33_genes.png'
...
kmeans_samples <- kmeans(Zscore_transposed, centers = 3, nstart = 25)
...
annotation_colors <- list(
  cluster_samples = c('1' = "#7FDC96", '2' ="#FF6D62", '3' ="#5F99EA"),
  cluster_genes = c('1' = 'brown', '2' = 'darkgreen', '3' = 'purple')
)
png(filename = heatmap_zscore_name, width = length(genes) / 1.6, height = nrow(Zscore_transposed_ordered) / 3.5, units = 'cm', res = 300)
...
png(survival_plot_name, height = 12, width = 16, units = 'cm', res = 300)
ggsurvplot(fit,
           pval = TRUE,
           conf.int = F,
           risk.table = F,
